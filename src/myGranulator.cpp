...
/*! \class myGranulator
 ...
 UPDATE BY: Graham Davis 2016
 Now includes functionality to control specific grain start/end parameters
 */
/***************************************************/
...
namespace stk {
    ...
    myGranulator :: myGranulator( void )
    {
    ...
    myGranulator :: myGranulator( unsigned int nVoices, std::string fileName, bool typeRaw )
    {
        this->setGrainParameters(); // use default values
        this->setRandomFactor();
        gStretch_ = 0;
    ...
    void myGranulator :: setStretch( unsigned int stretchFactor )
    {
        if ( stretchFactor <= 1 )
            gStretch_ = 0;
    ...
    void myGranulator :: setGrainParameters( unsigned int duration, unsigned int rampPercent,
                                         int offset, unsigned int delay, unsigned int start)
    {
        gDuration_ = duration;
        if ( gDuration_ == 0 ) {
            gDuration_ = 1;
            oStream_ << "myGranulator::setGrainParameters: duration argument cannot be zero ... setting to 1 millisecond.";
        ...
        if ( gRampPercent_ > 100 ) {
            gRampPercent_ = 100;
            oStream_ << "myGranulator::setGrainParameters: rampPercent argument cannot be greater than 100 ... setting to 100.";
            handleError( StkError::WARNING );
        }
        ...
        gDelay_ = delay;
        gStart_ = start;
        gEnd_ = gStart_ + duration;
    }
    
    void myGranulator :: setRandomFactor( StkFloat randomness )
    {
    ...
    void myGranulator :: openFile( std::string fileName, bool typeRaw )
    {
        ...
        message << "myGranulator::openFile: file = " << fileName << ", file frames = " << file.fileSize() << '.';
        handleError( message.str(), StkError::DEBUG_PRINT );
#endif
        
    }
    
    void myGranulator :: reset( void )
    {
        gPointer_ = 0;
        ...
        for ( unsigned int i=0; i<grains_.size(); i++ ) {
            ...
            grains_[i].startPointer = gStart_;
        }
    ...
    void myGranulator :: setVoices( unsigned int nVoices )
    {
#if defined(_STK_DEBUG_)
        std::ostringstream message;
        message << "myGranulator::setVoices: nVoices = " << nVoices << ", existing voices = " << grains_.size() << '.';
        handleError( message.str(), StkError::DEBUG_PRINT );
#endif
    ...
    void myGranulator :: calculateGrain( myGranulator::Grain& grain )
    {
        if ( grain.repeats > 0 ) {
            grain.repeats--;
        ...
        while ( grain.pointer >= data_.frames() || grain.pointer >= gEnd_) grain.pointer -= data_.frames();
        if ( grain.pointer <  gStart_ ) grain.pointer = gStart_;
        grain.startPointer = grain.pointer;
    }
    
    StkFloat myGranulator :: tick( unsigned int channel )
    {
#if defined(_STK_DEBUG_)
        if ( channel >= data_.channels() ) {
            oStream_ << "myGranulator::tick(): channel argument and soundfile data are incompatible!";
        ...
        for ( i=0; i<grains_.size(); i++ ) {
            ...
            if ( grains_[i].state > 0 ) {
                ...
                grains_[i].pointer++;
                if ( grains_[i].pointer >= data_.frames() || grains_[i].pointer >= gEnd_ || grains_[i].pointer < gStart_ )
                    grains_[i].pointer = gStart_;
            }
